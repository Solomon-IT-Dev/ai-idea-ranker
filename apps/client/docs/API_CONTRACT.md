...
The server is an Express API using Supabase JWT auth.
All requests use:
...
- `POST /v1/projects/:projectId/ideas:import`
  ...
  - server must enforce that imported ideas cannot be inserted into чужой project (projectId scoping + RLS)
