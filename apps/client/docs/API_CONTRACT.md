...
All protected requests include: `Authorization: Bearer <Supabase access token>`
...
- `DELETE /v1/projects/:id` deletes the project and related data (ideas, runs, playbook, chunks,
  artifacts, scores) via DB cascades.
...
- The UI treats these as known events (additional event types may appear and should be
  ignored/logged):
  - `run.started`
