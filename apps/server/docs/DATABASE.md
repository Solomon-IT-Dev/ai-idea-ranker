...
The server uses Supabase Postgres with Row-Level Security (RLS). User-scoped reads/writes are always
executed with an RLS-enforced, request-scoped Supabase client.
...
The Service Role client is for system tasks only (maintenance/background jobs). It bypasses RLS and
must not be used for user-scoped reads/writes.
...
- `src/db/sql/007_delete_policies.sql`
